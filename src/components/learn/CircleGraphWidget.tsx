...
export function CircleGraphWidget({ gridRange, equation, value, onChange, onSizeChange }: Props) {
  ...
  return (
    <div ref={containerRef} className="w-full h-full flex flex-col items-center justify-center" style={{ background: '#fdfaf4', borderRadius: 16, overflow: 'hidden' }}>
      <svg
      ...
      >
        ...
        {gridLines.map(v => (
          <g key={v}>
            <line x1={toX(v)} y1={pad / 2} x2={toX(v)} y2={size.h - pad / 2} stroke={v === 0 ? '#94a3b8' : '#e2e8f0'} strokeWidth={v === 0 ? 1.5 : 0.75} />
            <line x1={pad / 2} y1={toY(v)} x2={size.w - pad / 2} y2={toY(v)} stroke={v === 0 ? '#94a3b8' : '#e2e8f0'} strokeWidth={v === 0 ? 1.5 : 0.75} />
            {v !== 0 && (
              <>
                <text x={toX(v)} y={toY(0) + 14} textAnchor="middle" fontSize={9} fill="#94a3b8">{v}</text>
                <text x={toX(0) - 8} y={toY(v) + 4} textAnchor="end" fontSize={9} fill="#94a3b8">{v}</text>
              </>
            )}
          </g>
        ))}
        {/* Axis arrows */}
        <polygon points={`${toX(gridRange) + 4},${toY(0)} ${toX(gridRange)},${toY(0) - 5} ${toX(gridRange)},${toY(0) + 5}`} fill="#94a3b8" />
        <polygon points={`${toX(0)},${toY(gridRange) - 4} ${toX(0) - 5},${toY(gridRange)} ${toX(0) + 5},${toY(gridRange)}`} fill="#94a3b8" />
        <text x={toX(gridRange) + 10} y={toY(0) + 4} fontSize={11} fill="#64748b" fontStyle="italic">x</text>
        <text x={toX(0) + 6} y={toY(gridRange) - 6} fontSize={11} fill="#64748b" fontStyle="italic">y</text>
        ...
        {/* Coord labels */}
        <text x={cx + 10} y={cy - 10} fontSize={10} fill="#0f766e" fontWeight="600">
      ...
      {/* Equation label */}
      {equation && (
        <div className="mt-2 mb-3 text-sm font-medium text-slate-600" style={{ fontFamily: 'Georgia, serif', letterSpacing: 0.3 }}>
          {equation}
        </div>
      )}
    </div>
